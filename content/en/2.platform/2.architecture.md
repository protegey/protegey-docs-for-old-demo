...
Protegey exposes three endpoints that map to distinct stages of the fraud intelligence lifecycle:
...
Raw Input → POST /evaluate → (resolve_entity: true) → Entity → POST /risk/assess
           ...
           Immediate verdict                               Full intelligence package
                                 ↑
                       POST /signals (ingest)
                       feeds entity graph over time
```

| Layer | Endpoint | Type | Purpose |
|---|---|---|---|
| Ingestion | `POST /signals` | Stateful | Feed the entity graph with behavioral events |
| Evaluation | `POST /evaluate` | Stateless | Score raw identifiers in < 100ms |
| Assessment | `POST /risk/assess` | Stateful | Full entity intelligence package in < 500ms |

See the [Intelligence Model](/partners/api-reference/api-intelligence-model) for when to use each endpoint.
...
Protegey uses a progressive onboarding model that prioritizes fast sandbox access while keeping production gated behind verification.

### First-Time Access (Sandbox-First)

1. Partner resets password and logs in for the first time.
2. Partner sees a short, non-blocking intro (platform tour, sandbox limits, support links). The intro only verifies basic details and does not collect onboarding data.
3. Sandbox access is granted immediately.

### Production Access (Gated)

When a partner requests production access, they complete the full onboarding process (compliance, identity, billing, and technical validation). Production access is granted only after onboarding is approved.

### Sandbox vs Production Access

- **Access timing**: Sandbox access is granted immediately after first login; production access is granted only after onboarding approval.
- **Requirements**: Sandbox requires a short intro; production requires compliance, identity, billing, and technical validation.
- **Purpose**: Sandbox is for exploration and integration; production is for live traffic and operational monitoring.

**Why this model**:

- **Faster activation**: Partners can explore and integrate immediately.
- **Clear separation**: Sandbox and production remain isolated by schema and policy.
- **Controlled risk**: Production access remains governed by onboarding checks.
...
_Latency Impact: Near Zero_

---

## Phase M: Monitoring Layer

The monitoring layer provides two health endpoints:

- **`/ready`** — liveness probe for load balancers. Checks database connectivity only. Target: < 100ms.
- **`/health`** — deep health check covering database, Redis, queue workers, Reverb WebSocket, graph cache, and OSINT ingestion. Response cached 30 seconds.

Queue monitoring is available in the admin panel with per-queue statistics, failed job management, and retry tooling.

## Support Module Domain

The `Support` domain (`app/Domain/Support/`) handles in-platform messaging between partners and the Protegey team. It integrates with Laravel Reverb for real-time message delivery and stores threads, messages, and attachments with partner-scoped access control.

## Graph Intelligence Layer

The hot graph layer (PostgreSQL `graph_nodes` + `graph_edges`, Redis adjacency cache) maps entity relationships and propagates risk through networks in real time. The graph's `network_score` contributes 40% of the final fused risk score. BFS traversal to depth 2 completes in < 5ms against the Redis cache.
